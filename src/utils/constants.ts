export const iconList = [
  {
    label: "Vue",
    icon: "https://cdn.simpleicons.org/vue.js",
  },
  {
    label: "React",
    icon: "https://cdn.simpleicons.org/react",
  },
  {
    label: "Javascript",
    icon: "https://cdn.simpleicons.org/javascript",
  },
  {
    label: "Typescript",
    icon: "https://cdn.simpleicons.org/typescript",
  },
  {
    label: "Html5",
    icon: "https://cdn.simpleicons.org/html5",
  },
  {
    label: "Css3",
    icon: "https://cdn.simpleicons.org/css3",
  },
  {
    label: "Node",
    icon: "https://cdn.simpleicons.org/nodedotjs",
  },
  {
    label: "Npm",
    icon: "https://cdn.simpleicons.org/npm",
  },
  {
    label: "Angular",
    icon: "https://cdn.simpleicons.org/angular",
  },
  {
    label: "Electron",
    icon: "https://cdn.simpleicons.org/electron",
  },
  {
    label: "Redux",
    icon: "https://cdn.simpleicons.org/redux",
  },
  {
    label: "Flutter",
    icon: "https://cdn.simpleicons.org/flutter",
  },
  {
    label: "Nuxt",
    icon: "https://cdn.simpleicons.org/nuxtdotjs",
  },
  {
    label: "Next",
    icon: "https://cdn.simpleicons.org/nextdotjs",
  },
  {
    label: "Webpack",
    icon: "https://cdn.simpleicons.org/webpack",
  },
  {
    label: "Vite",
    icon: "https://cdn.simpleicons.org/vite",
  },
  {
    label: "Tailwind",
    icon: "https://cdn.simpleicons.org/tailwindcss",
  },
  {
    label: "Git",
    icon: "https://cdn.simpleicons.org/git",
  },
  {
    label: "Chrome",
    icon: "https://cdn.simpleicons.org/googlechrome",
  },
  {
    label: "Github",
    icon: "https://cdn.simpleicons.org/github",
  },
  {
    label: "Gitlab",
    icon: "https://cdn.simpleicons.org/gitlab",
  },
  {
    label: "Docker",
    icon: "https://cdn.simpleicons.org/docker",
  },
  {
    label: "Figma",
    icon: "https://cdn.simpleicons.org/figma",
  },
  {
    label: "Java",
    icon: "/icons/java.png",
  },
  {
    label: "Python",
    icon: "https://cdn.simpleicons.org/python",
  },
  {
    label: "C++",
    icon: "https://cdn.simpleicons.org/cplusplus",
  },
  {
    label: "Windows",
    icon: "/icons/windows.png",
  },
  {
    label: "Linux",
    icon: "https://cdn.simpleicons.org/linux",
  },
  {
    label: "MacOS",
    icon: "https://cdn.simpleicons.org/macos",
  },
  {
    label: "Safari",
    icon: "https://cdn.simpleicons.org/safari",
  },
  {
    label: "Firefox",
    icon: "https://cdn.simpleicons.org/firefox",
  },
  {
    label: "Mongodb",
    icon: "https://cdn.simpleicons.org/mongodb",
  },
  {
    label: "Mysql",
    icon: "https://cdn.simpleicons.org/mysql",
  },
  {
    label: "Tensorflow",
    icon: "https://cdn.simpleicons.org/tensorflow",
  },
  {
    label: "Stackoverflow",
    icon: "https://cdn.simpleicons.org/stackoverflow",
  },
  {
    label: "Vscode",
    icon: "/icons/vscode.png",
  },
];

export const SkeletonTemplates = ['skeleton-one', 'skeleton-two', 'skeleton-three', 'skeleton-four', 'skeleton-five'];

export const gradientColors = [
  'linear-gradient(310deg,rgb(214,233,255),rgb(214,229,255),rgb(209,214,255),rgb(221,209,255),rgb(243,209,255),rgb(255,204,245),rgb(255,204,223),rgb(255,200,199),rgb(255,216,199),rgb(255,221,199))',
  'linear-gradient(160deg,rgb(204,251,252),rgb(197,234,254),rgb(189,211,255))',
  'linear-gradient(150deg,rgb(255,242,158),rgb(255,239,153),rgb(255,231,140),rgb(255,217,121),rgb(255,197,98),rgb(255,171,75),rgb(255,143,52),rgb(255,115,33),rgb(255,95,20),rgb(255,87,15))',
  'linear-gradient(345deg,rgb(211,89,255),rgb(228,99,255),rgb(255,123,247),rgb(255,154,218),rgb(255,185,208),rgb(255,209,214),rgb(255,219,219))',
  'linear-gradient(140deg, rgb(255, 100, 50) 12.8%, rgb(255, 0, 101) 43.52%, rgb(123, 46, 255) 84.34%)',
  'linear-gradient(150deg, #ef32d9, #89fffd)',
  'linear-gradient(25deg, #7c2df3, #7167f7, #5992fb, #00b9ff)',
  'linear-gradient(25deg, #00b7f7, #a2b7b5, #d5b871, #f6b900)',
  'linear-gradient(150deg,rgb(0,224,245),rgb(31,158,255),rgb(51,85,255))',
  'linear-gradient(150deg,rgb(0,176,158),rgb(19,77,93),rgb(16,23,31))',
  'linear-gradient(150deg,rgb(95,108,138),rgb(48,59,94),rgb(14,18,38))',
];

export const singleColors = [
  '#ff4500',
  '#ff8c00',
  '#ffd700',
  '#90ee90',
  '#00ced1',
  '#1e90ff',
  '#c71585',
  'rgba(255, 69, 0, 0.68)',
  'rgb(255, 120, 0)',
  'hsv(51, 100, 98)',
  'hsva(120, 40, 94, 0.5)',
  'hsl(181, 100%, 37%)',
  'hsla(209, 100%, 56%, 0.73)',
  '#c7158577',
];

export const fontFamilys = [
  {
    label: '微软雅黑',
    value: 'Microsoft YaHei',
  },
  {
    label: '仿宋',
    value: 'fangsong',
  },
  {
    label: 'cursive',
    value: 'cursive',
  },
  {
    label: 'fantasy',
    value: 'fantasy',
  },
];